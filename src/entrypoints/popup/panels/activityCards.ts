import type { Activity, MomAction } from '@/messaging/messages';
import {
  describeEta,
  describePaused,
  describePhase,
...
import { button, el, track } from '@/ui/dom';

/**
 * Rendering the "Now" list.
 *
 * Kept apart from the panel that polls for it because this is the only part of
 * the popup with real logic in it, and it is worth being able to test every
...
export interface ActivityHandlers {
  ...
  /** Pause, resume or abandon a summarisation run. */
  readonly onMom: (sessionId: string, action: MomAction) => void;
}
...
function card(a: Activity, on: ActivityHandlers): HTMLElement {
  ...
  switch (a.kind) {
    ...
    case 'processing': {
      ...
      top.append(
        chip(a.paused ? 'hold' : 'work', a.paused ? 'Paused' : 'Writing minutes'),
        el('span', 'card-time', `${progressPercent(a.progress)}%`),
      );

      // The estimate rides with the phase line — the top row is for the
      // headline number only. A paused run gets no estimate: there is nothing
      // running to estimate, and a countdown that never moves reads as stuck.
      const eta = describeEta(a.progress.etaMs);
      const phase = describePhase(a.progress);
      let detail: string;
      if (a.paused) detail = describePaused(a.progress);
      else detail = eta === null ? phase : `${phase} · ${eta}`;
      ...
      // Cancel is the quieter of the two on purpose: pausing is reversible and
      // cancelling throws away every chunk summarised so far.
      const controls = el('div', 'btn-pair');
      controls.append(
        a.paused
          ? button('Resume', 'btn tiny', () => on.onMom(a.sessionId, 'resume'))
          : button('Pause', 'btn tiny', () => on.onMom(a.sessionId, 'pause')),
        button('Cancel', 'btn tiny ghost', () => on.onMom(a.sessionId, 'cancel')),
      );

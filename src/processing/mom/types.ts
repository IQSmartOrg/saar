import type { ActionItem, Decision, Topic } from '@/minutes/types';
 ...
 *
 * Says what is already done rather than what is happening, because nothing is:
 * `describePhase` reads as activity ("Reading the transcript…"), which is
 * exactly the wrong thing to show beside a Resume button.
 */
export function describePaused(p: MomProgress): string {
  switch (p.phase) {
    case 'mapping':
      // `total` counts the merge too, so subtract it to name transcript parts.
      return p.done === 0
        ? `Paused before the first of ${p.total - 1} parts`
        : `Paused after part ${p.done} of ${p.total - 1}`;
    case 'reducing':
      return 'Paused before the final merge';
    default:
      return 'Paused';
  }
}

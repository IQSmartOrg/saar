...
import { botTabUrl, isBotTab, parseMeetingCode } from '@/meet/meetingCode';
...
describe('botTabUrl', () => {
  it('carries the account index and the session id', () => {
    expect(botTabUrl('abc-defg-hij', 1, 's1')).toBe(
      'https://meet.google.com/abc-defg-hij?authuser=1&saarSession=s1',
    );
  });

  it('produces a URL isBotTab recognises', () => {
    expect(isBotTab(botTabUrl('abc-defg-hij', 0, 's1'))).toBe(true);
  });
});
